...
const filterValues = {
  ...
  after: value => ({
    gt: dayjs
      .utc(dateStringFormatter(value))
      .add(1, "day")
      .add(utcOffset(), "minute")
      .format()
  }),
  afterExact: value => ({
    gt: dayjs.utc(dateStringFormatter(value)).format()
  }),
  before: value => ({
    lt: dayjs
      .utc(dateStringFormatter(value))
  ...
  beforeExact: value => ({
    lte: dayjs.utc(dateStringFormatter(value)).format()
  }),
  on: value => ({
    gt: dayjs
      .utc(dateStringFormatter(value))
...
function formatBetweenDatesFilter(body, filter) {
  ...
  function dateOrNullQuery(field) {
    body.query("bool", b =>
      b
        .orQuery("bool", c => c.notQuery("exists", field))
        .orQuery(
          "range",
          field,
          getFilterValue({ value: currentDate, comparison: "afterExact" })
        )
    )
  }
  ...
  function dateOnlyQuery(field, comparison, type = "query") {
    body[type](
      "range",
      field,
      getFilterValue({ value: currentDate, comparison })
    )
  }

  if (filter.attribute === "sunset") {
    isActive
      ? dateOrNullQuery("sunsetAt")
      : dateOnlyQuery("sunsetAt", "beforeExact")
...
//   node:{
//     url: process.env.VUE_APP_ELASTIC_HOST,
//     headers: {
...
export default class ElasticAdapter extends DataProcessor {
  constructor(params) {
    ...
    const url = new URL(process.env.VUE_APP_ELASTIC_HOST)
    ...
    this.client = new Client({
      host: {
        ...
        port: 443,
        headers: {
          Authorization: "Bearer " + params.component.$accessToken
        }
      },
      apiVersion:'6.8'
    })
